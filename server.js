...
const server=http.createServer(async(req,res)=>{try{
 ...
 if(p.startsWith('/api/')&&p!=='/api/site'&&p!=='/api/login'&&!p.startsWith('/api/admin')){const md=await read();if(md.system?.maintenance?.enabled===true)return send(res,503,{ok:false,error:'Ludo Baji Website & App Update চলছে। এখন প্রবেশ করা যাবে না।'});}
